...
def epoch_now(): return int(datetime.datetime.now().timestamp())
def epoch_days_ago(days): return int((datetime.datetime.now() - datetime.timedelta(days=days)).timestamp())
...
def build_dataset_for_code(tag_value: str):
    ...
    host_enabled = {h["hostid"]: (str(h.get("status", "0")) == "0") for h in hosts}

    now, start = epoch_now(), epoch_days_ago(DAYS)
    ...
    for hostid in hostids:
        ...
        problem_count, downtime_total = 0, 0
        for triggerid in icmp_trigger_ids:
            events = zabbix_api('event.get', {
                ...
                "value": 1, "sortfield": ["clock"], "sortorder": "ASC"
            })
            for ev in events:
                ...
                if r_evid and str(r_evid) != "0":
                    ...
                    if resolved_event:
                        start_ts, end_ts = int(ev['clock']), int(resolved_event[0]['clock'])
                        down_seconds = end_ts - start_ts
                        if down_seconds > 0:
                            downtime_total += down_seconds; problem_count += 1
                            ack_time, ack_notes = "", ""
                            ...
                            problem_details.append({
                                "Host": hostmap[hostid], "Severity": sev, "Status": "RESOLVED",
                                "Duration": fmt_duration(down_seconds),
                                "Problem": trig.get("description", "Unavailable by ICMP ping"),
                                "Alert Time": fmt_time(start_ts), "Acknowledged time": ack_time,
                                "Recovery time": fmt_time(end_ts), "Notes": ack_notes
                            })

        availability = 0.000
        icmp_items = zabbix_api('item.get', {
            "output": ["itemid"], "hostids": [hostid], "search": {"key_": "icmpping"}, "limit": 1
        })
        if icmp_items:
            itemid = icmp_items[0]["itemid"]
            history = zabbix_api('history.get', {"output": "extend", "history": 0,
                                                 "itemids": [itemid],"time_from": start,
                                                 "time_till": now,"limit": 100000})
            values = [float(h['value']) for h in history]
            if not values:
                history = zabbix_api('history.get', {"output": "extend","history": 3,
                                                     "itemids": [itemid],"time_from": start,
                                                     "time_till": now,"limit": 100000})
                values = [float(h['value']) for h in history]
            if values:
                availability = 100 * sum(values) / len(values)

        results.append({
            "Hostname": hostmap[hostid], "Availability %": round(availability, 2),
            "Problems Raised": problem_count, "Total Downtime (min)": round(downtime_total/60),
            "Enabled": "Yes" if host_enabled.get(hostid, True) else "No"
        })

    df = pd.DataFrame(results)
    if not df.empty: df = df.sort_values(by=["Hostname"], kind="stable").reset_index(drop=True)
    return df, problem_details

def write_sheet(df, problem_details, writer, summary, sheet_title):
    ...
    bold = workbook.add_format({"bold": True}); pct2 = workbook.add_format({"num_format": "0.00"})
    int_fmt = workbook.add_format({"num_format": "0"})
    worksheet.write("A1", f"SLA Availability Report — {sheet_title}", h1)
    worksheet.write("A3", f"Time Frame: last {DAYS} days")
    worksheet.write("B6", "Enabled devices", bold); worksheet.write_number("C6", summary["enabled_devices"], int_fmt)
    worksheet.write("D6", "Total devices", bold); worksheet.write_number("E6", summary["total_devices"], int_fmt)
    worksheet.write("D2", "Avg Availability (Enabled)", bold); worksheet.write_number("E2", summary["avg_enabled_availability"], pct2)
    worksheet.write("B4", "Total problems (all)", bold); worksheet.write_number("C4", summary["problems_total"], int_fmt)
    worksheet.write("B5", "Total downtime (min, all)", bold); worksheet.write_number("C5", summary["downtime_total_min"], int_fmt)
    ...
    for col_idx,col in enumerate(details_cols): worksheet.write(details_start_row, col_idx, col, bold)
    for r_idx,row in df_details.iterrows():
        for c_idx,col in enumerate(details_cols): worksheet.write(details_start_row+1+r_idx, c_idx, row[col])
...
def write_summary_sheet(writer, rows):
    sheet_name = "Summary"
    df_sum = pd.DataFrame(rows, columns=["Group","Avg Availability (Enabled)"])
    df_sum.sort_values(by="Group", inplace=True, kind="stable"); df_sum.reset_index(drop=True,inplace=True)
    df_sum.to_excel(writer, sheet_name=sheet_name, startrow=0, index=False)
    workbook, ws = writer.book, writer.sheets[sheet_name]
    h1 = workbook.add_format({"bold": True, "font_size": 16}); pct2 = workbook.add_format({"num_format": "0.00"})
    ...
    ws.set_column(0,0,min(50,max(15,int(max_w0*1.1)))); ws.set_column(1,1,22,pct2); ws.freeze_panes(1,0)

def main():
    with open(SLA_CODES_FILE,"r",encoding="utf-8") as f: entries = (yaml.safe_load(f) or {}).get("sla_report_codes",[])
    if not entries: raise SystemExit(f"No 'sla_report_codes' found in {SLA_CODES_FILE}")
    with pd.ExcelWriter(OUTPUT_FILE, engine="xlsxwriter") as writer:
        writer.book.add_worksheet("Summary")
        summary_rows = []
        for entry in entries:
            code,name = entry.get("code"), entry.get("name") or entry.get("code")
            ...
            if not df_full.empty:
                ...
                if "Enabled" in df_export.columns: df_export.drop(columns=["Enabled"], inplace=True)
            else: df_export = df_full
            if df_export.empty: continue
